...
We created two regression models and plotted them to answer this question. 
  
For our second multivariate plot, we wanted to examine the correlation between "download speeds" and "satisfaction" by each ISP. We specifically wanted to answer the following question: 

"Are higher download and upload speeds correlated with higher levels of satisfaction?" ###WE NEED TO CONFIRM THIS IS CORRECT###

We used a correlation plot to answer this question. 
...
#Create a box plot for each ISP, illustrating the ranges of their dowload speeds 
box1 = ggplot(Internet, aes(x = ISP, y=Download.Speed))

#Add information regarding the colors of the plot, and then flip it so the axes are reversed 
box2 = box1 + geom_boxplot(color='black',fill='white',
                           outlier.color='black',
                           outlier.size=2) + coord_flip()
                           
#Add a title, caption, and change the labels on each axis. 
...
#Change the scale of each axis so that we show the exact number of tick marks that we'd like 
...
The boxplot shows that GorgeNet has the least spread out download speed distribution. It means that there is not a huge variation in download speeds among GorgeNet's customers. In contrast, Wave provides the highest range of download speeds, with some customers receiving less than 10 Mbps and some others more than 50 Mbps. Century Link also has a small download speed range; however, the plot indicates the presence of an outlier.


```{r message=FALSE, warning=FALSE}
#Create a second box plot for each ISP, illustrating the ranges of their upload speeds 
...
We expected to find a linear relationship between both variables, advertised and actual Internet speeds. This means that the advertised speed would correspond to the actual speed provides.
...
#Create a base plot of Subscribed Speeds versus Upload Speeds 
...
#Format each text element, as before 
...
#Call three libraries that would help us make dot and whisker plots as well as 
library(dotwhisker)
library(broom)
library(dplyr)
...
```{r message= FALSE, warning = FALSE}
#Create a second model of regression, this time with a fourth variable: Subscribed Speeds. 
